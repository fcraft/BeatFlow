...
TEMPLATES = [
    ...
    {
        "id": "exercise_treadmill_moderate",
        "name": "平板运动试验（Bruce Stage 2）",
        "category": "normal",
        "description": "跑步机中等负荷（4.0 km/h · 12% 坡度），心率约 130bpm，轻度运动伪影，模拟 Bruce 方案第二阶段",
    ...
    {
        "id": "exercise_treadmill_peak",
        "name": "平板运动试验（Bruce Stage 4）",
        "category": "normal",
        "description": "跑步机高负荷（6.8 km/h · 16% 坡度），心率约 160bpm，强运动伪影，S1/S2 幅度增大，模拟 Bruce 方案第四阶段",
    ...
    # ── 新增模板 ─────────────────────────────────────────────────────────────
    {
        ...
        "name": "舒张期杂音（主动脉瓣关闭不全）",
        "category": "valvular",
        "description": "S2 后递减型舒张期杂音，高调吹风样，常见于主动脉瓣关闭不全",
        "params": dict(ecg_rhythm="normal", heart_rate=78, heart_rate_std=2, noise_level=0.01,
                       duration=12, generate_pcg=True,
                       pcg_abnormalities=["murmur_diastolic"],
                       s1_amplitude=1.0, s2_amplitude=0.8, pcg_noise_level=0.005),
    },
    {
        "id": "s4_gallop",
        "name": "S4 奔马律（高血压性心脏病）",
        "category": "valvular",
        "description": "S1 前额外低频 S4 音，提示心室顺应性下降，常见于高血压性心脏病和肥厚型心肌病",
        "params": dict(ecg_rhythm="normal", heart_rate=80, heart_rate_std=2, noise_level=0.01,
                       duration=12, generate_pcg=True,
                       pcg_abnormalities=["s4_gallop"],
                       s1_amplitude=1.0, s2_amplitude=0.7, pcg_noise_level=0.005),
    },
    {
        "id": "split_s2",
        "name": "S2 分裂",
        "category": "valvular",
        "description": "S2 分裂为 A2 和 P2 两个成分，常见于吸气时的生理性分裂或房间隔缺损的固定分裂",
        "params": dict(ecg_rhythm="normal", heart_rate=72, heart_rate_std=2, noise_level=0.01,
                       duration=12, generate_pcg=True,
                       pcg_abnormalities=["split_s2"],
                       s1_amplitude=1.0, s2_amplitude=0.7, pcg_noise_level=0.004),
    },
    {
        "id": "combined_valvular",
        "name": "复合瓣膜病（收缩期杂音 + S3）",
        "category": "valvular",
        "description": "收缩期杂音合并 S3 奔马律，提示二尖瓣关闭不全伴心力衰竭，适合多病变组合教学",
        "params": dict(ecg_rhythm="tachycardia", heart_rate=105, heart_rate_std=3, noise_level=0.01,
                       duration=15, generate_pcg=True,
                       pcg_abnormalities=["murmur_systolic", "s3_gallop"],
                       s1_amplitude=0.9, s2_amplitude=0.65, pcg_noise_level=0.006),
    },
    {
        "id": "elderly_resting",
        "name": "老年人静息心率",
        "category": "normal",
        "description": "60bpm 偏低静息心率，低噪声，模拟老年人安静状态下的典型心电心音",
        "params": dict(ecg_rhythm="normal", heart_rate=60, heart_rate_std=1.5, noise_level=0.008,
                       duration=15, generate_pcg=True, pcg_abnormalities=[],
                       s1_amplitude=0.85, s2_amplitude=0.6, pcg_noise_level=0.004),
    },
    {
        "id": "anxiety_fever",
        "name": "焦虑/发热心率",
        "category": "normal",
        "description": "110bpm 轻度心动过速，稍高噪声，模拟焦虑、发热或紧张状态下的心率加快",
        "params": dict(ecg_rhythm="tachycardia", heart_rate=110, heart_rate_std=4, noise_level=0.015,
                       duration=10, generate_pcg=True, pcg_abnormalities=[],
                       s1_amplitude=1.05, s2_amplitude=0.75, pcg_noise_level=0.005),
    },
    {
        "id": "frequent_pvc",
        "name": "频发室性早搏（>30%）",
        "category": "arrhythmia",
        "description": "PVC 比例 >30%，频繁出现宽大畸形 QRS 和代偿间歇，常需临床干预",
        "params": dict(ecg_rhythm="pvc", heart_rate=75, heart_rate_std=4, pvc_ratio=0.35,
                       noise_level=0.01, duration=15, generate_pcg=True, pcg_abnormalities=[],
                       s1_amplitude=0.7, s2_amplitude=0.55, pcg_noise_level=0.005),
    },
    {
        "id": "exercise_cycle_peak",
        "name": "功率自行车运动试验（峰值负荷）",
        "category": "normal",
        "description": "功率自行车峰值负荷（200W），心率约 175bpm，强运动伪影和摩擦噪声，S1/S2 幅度显著增大",
        "params": dict(ecg_rhythm="tachycardia", heart_rate=175, heart_rate_std=8, noise_level=0.025,
                       duration=15, generate_pcg=True, pcg_abnormalities=[],
                       s1_amplitude=1.4, s2_amplitude=1.1, pcg_noise_level=0.01,
                       stethoscope_mode=True, pcg_sample_rate=8000,
                       exercise_intensity=0.85),
    },
]
